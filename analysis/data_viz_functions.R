...
make_cluster_scatterplot <- function(results_df, cluster_var, trial_name) {
  # Set plotting defaults, assuming set_urbn_defaults is a function from your libraries
  set_urbn_defaults(style = "print")
  
  # Create the scatterplot
  cluster_plot <- ggplot(data = results_df, aes(x = total_police_scale, y = total_citizen_scale)) +
    geom_point(aes(color = !!sym(cluster_var)), alpha  = .75) +
    scale_x_continuous(name = "Police-Initiated Activity: Stop and Arrest Rate (Standard Deviations from Mean)",
                       limits = c(-2, 13)) +
    scale_y_continuous(name = "Resident-Initiated Activity: Calls for Service Rate (Standard Deviations from Mean)",
  ...
  ggsave(here("images", paste(trial_name, cluster_var, "cluster_plot.png", sep = "_")), 
         plot = cluster_plot, width = 6, height = 4.7, units = "in")
...
make_mean_by_cluster_barplot <- function(vars, lbls, file_name, trial_name,
  ...
  file_path <- here::here("images", paste(trial_name, file_name, sep = "_"))
  
  # Saving the plot with ggsave()
  ggsave(filename = file_path, plot = mean_by_cluster_plot, width = 8, height = 6, units = "in")
...
make_per_capita_barplot <- function(vars, denom_var, lbls, file_name,
  ...
  ggsave(here("images", 
                paste(trial_name, file_name, sep = "_")), plot = per_capita_plot,
